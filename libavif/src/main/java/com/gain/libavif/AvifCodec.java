...
public class AvifCodec {
  static {
    try {
      System.loadLibrary("avif_sample");
  ...
  public static boolean isAvifImage(ByteBuffer buffer) {
    return AvifCodec.isAvifImage(buffer, buffer.remaining());
  }
   ...
   * @param rgbaData The rgba data to be encoded.
   * @param length
   * @param width
   * @param height
   * @return AVIF image's content
   */
  public static native byte[] encodeRGBA8888(ByteBuffer rgbaData, int length, int width, int height);

  /**
   * Encode the Y420 data into AVIF image.
   * @param yData
   * @param uData
   * @param vData
   * @param strideY
   ...
   * @return AVIF image's content
   */
  public static native byte[] encodeY420(ByteBuffer yData,
                                        ByteBuffer uData,
                                        ByteBuffer vData,
                                        int strideY,
                                        int strideU,
